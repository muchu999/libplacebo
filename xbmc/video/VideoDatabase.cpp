...
static std::vector<std::string> LibplaceboColumnslist = {
								   ...
								   "PlaceboTargetContrast",
								   "PlaceboNvRtxPipelineEnabled",
								   "PlaceboNvSuperResolutionEnabled",
								   "PlaceboNvRtxHdrEnabled",
								   "PlaceboSdrTargetContrast",
...
bool CVideoDatabase::AddLibplaceboColumnsToSettingsTable(int idFile, const CVideoSettings& settings)
{
  try
  {
    ...
    //if (a->records.size() < 29+LibplaceboColumnslist.size()) // unsafe...
    {
      ...
      //cl simplify?
      static std::vector<std::string> list = LibplaceboColumnslist;
      for (int i = 0; i < numColumns; i++)
      {
        std::erase(list, a->records[i]->at(col).get_asString()); // remove what exists from the list 
      }
      ...
      for (int i = 0; i < list.size(); i++)
      {
		...
		if (list[i] == "PlaceboSkinZoomPosition") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSkinZoomPosition                  integer NOT NULL DEFAULT %i", vs.m_PlaceboSkinZoomPosition); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboLutFilename") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboLutFilename                            text NOT NULL DEFAULT '%s'", vs.m_PlaceboLutFilename.c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDisplayHdrPeakLuminance") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDisplayHdrPeakLuminance    float NOT NULL DEFAULT   %f", vs.m_PlaceboDisplayHdrPeakLuminance); m_pDS->exec(strSQL2); }
		...
		if (list[i] == "PlaceboTargetContrast") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboTargetContrast                      integer NOT NULL DEFAULT %i", vs.m_PlaceboTargetContrast); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboNvRtxPipelineEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboNvRtxPipelineEnabled          bool NOT NULL DEFAULT %i", vs.m_PlaceboNvRtxPipelineEnabled); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboNvSuperResolutionEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboNvSuperResolutionEnabled  bool NOT NULL DEFAULT %i", vs.m_PlaceboNvSuperResolutionEnabled); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboNvRtxHdrEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboNvRtxHdrEnabled                    bool NOT NULL DEFAULT %i", vs.m_PlaceboNvRtxHdrEnabled); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrTargetContrast") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrTargetContrast                integer NOT NULL DEFAULT %i", vs.m_PlaceboSdrTargetContrast); m_pDS->exec(strSQL2); }
		...
		if(list [i] == "PlaceboBrightnessSdrSdr") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboBrightnessSdrSdr                        float NOT NULL DEFAULT %f", vs.m_PlaceboBrightnessSdrSdr); m_pDS->exec(strSQL2); }
		if(list [i] == "PlaceboContrastSdrSdr") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboContrastSdrSdr                            float NOT NULL DEFAULT %f", vs.m_PlaceboContrastSdrSdr); m_pDS->exec(strSQL2); }
		if(list [i] == "PlaceboBrightnessHdrHdr") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboBrightnessHdrHdr                        float NOT NULL DEFAULT %f", vs.m_PlaceboBrightnessHdrHdr); m_pDS->exec(strSQL2); }
		if(list [i] == "PlaceboContrastHdrHdr") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboContrastHdrHdr                            float NOT NULL DEFAULT %f", vs.m_PlaceboContrastHdrHdr); m_pDS->exec(strSQL2); }
		if(list [i] == "PlaceboBrightnessHdrSdr") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboBrightnessHdrSdr                        float NOT NULL DEFAULT %f", vs.m_PlaceboBrightnessHdrSdr); m_pDS->exec(strSQL2); }
		...
		if (list[i] == "PlaceboSdrColorMapGamutExpansion") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrColorMapGamutExpansion integer NOT NULL DEFAULT %i", vs.m_PlaceboSdrColorMapGamutExpansion); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrColorMapIntent") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrColorMapIntent                text NOT NULL DEFAULT '%s'", CPLHelper::getColorMapIntentDescriptionFromIndex(vs.m_PlaceboSdrColorMapIntent).c_str()); m_pDS->exec(strSQL2);}
		if (list[i] == "PlaceboSdrColorMapGamutMapping") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrColorMapGamutMapping             text NOT NULL DEFAULT '%s'", vs.m_PlaceboSdrColorMapGamutMapping == -1 ? "disabled" : pl_gamut_map_functions [vs.m_PlaceboSdrColorMapGamutMapping]->description == nullptr ? "''" : pl_gamut_map_functions [vs.m_PlaceboSdrColorMapGamutMapping]->description); m_pDS->exec(strSQL2);}
		if (list[i] == "PlaceboSdrColorMapToneMapping") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrColorMapToneMapping               text NOT NULL DEFAULT '%s'", vs.m_PlaceboSdrColorMapToneMapping == -1 ? "disabled" : pl_tone_map_functions [vs.m_PlaceboSdrColorMapToneMapping]->description == nullptr ? "''" : pl_tone_map_functions [vs.m_PlaceboSdrColorMapToneMapping]->description); m_pDS->exec(strSQL2);}
		if (list[i] == "PlaceboSdrToneConstantExposure") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantExposure             float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantExposure); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantKneeAdaptation") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantKneeAdaptation float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantKneeAdaptation); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantKneeDefault") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantKneeDefault       float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantKneeDefault); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantKneeMaximum") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantKneeMaximum       float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantKneeMaximum); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantKneeMinimum") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantKneeMinimum       float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantKneeMinimum); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantKneeOffset") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantKneeOffset         float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantKneeOffset); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantLinearKnee") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantLinearKnee         float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantLinearKnee); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrToneConstantReinhardContrast") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrToneConstantReinhardContrast float NOT NULL DEFAULT %f", vs.m_PlaceboSdrToneConstantReinhardContrast); m_pDS->exec(strSQL2); }
		...
		if (list[i] == "PlaceboSdrGamutConstantsSoftclipDesat") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrGamutConstantsSoftclipDesat           float NOT NULL DEFAULT %f", vs.m_PlaceboSdrGamutConstantsSoftclipDesat); m_pDS->exec(strSQL2); }
		if (list[i] == "PlaceboSdrGamutConstantsSoftclipKnee") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSdrGamutConstantsSoftclipKnee             float NOT NULL DEFAULT %f", vs.m_PlaceboSdrGamutConstantsSoftclipKnee); m_pDS->exec(strSQL2); }
        ...
        if (list[i] == "PlaceboUpscaler") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboUpscaler               text NOT NULL DEFAULT '%s'", vs.m_PlaceboUpscaler == -1 ? "disabled" : pl_filter_configs[vs.m_PlaceboUpscaler]->description == nullptr ? "''" : pl_filter_configs[vs.m_PlaceboUpscaler]->description); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDownscaler") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDownscaler           text NOT NULL DEFAULT '%s'", vs.m_PlaceboDownscaler == -1 ? "disabled" : pl_filter_configs[vs.m_PlaceboDownscaler]->description == nullptr ? "''" : pl_filter_configs[vs.m_PlaceboDownscaler]->description); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboPlaneUpscaler") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboPlaneUpscaler     text NOT NULL DEFAULT '%s'", vs.m_PlaceboPlaneUpscaler == -1 ? "disabled" : pl_filter_configs[vs.m_PlaceboPlaneUpscaler]->description == nullptr ? "''" : pl_filter_configs[vs.m_PlaceboPlaneUpscaler]->description); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboPlaneDownscaler") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboPlaneDownscaler text NOT NULL DEFAULT '%s'", vs.m_PlaceboPlaneDownscaler == -1 ? "disabled" : pl_filter_configs[vs.m_PlaceboPlaneDownscaler]->description == nullptr ? "''" : pl_filter_configs[vs.m_PlaceboPlaneDownscaler]->description); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboFrameMixer") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboFrameMixer           text NOT NULL DEFAULT '%s'", vs.m_PlaceboFrameMixer == -1 ? "disabled" : pl_filter_configs[vs.m_PlaceboFrameMixer]->description == nullptr ? "''" : pl_filter_configs[vs.m_PlaceboFrameMixer]->description); m_pDS->exec(strSQL2); }
        ...
        if (list[i] == "PlaceboDebandGrainNeutral0") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDebandGrainNeutral0 float NOT NULL DEFAULT    %f", vs.m_PlaceboDebandGrainNeutral0); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDebandGrainNeutral1") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDebandGrainNeutral1 float NOT NULL DEFAULT    %f", vs.m_PlaceboDebandGrainNeutral1); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDebandGrainNeutral2") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDebandGrainNeutral2 float NOT NULL DEFAULT    %f", vs.m_PlaceboDebandGrainNeutral2); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDebandIterations") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDebandIterations    integer NOT NULL DEFAULT  %i", vs.m_PlaceboDebandIterations); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDebandRadius") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDebandRadius        float NOT NULL DEFAULT    %f", vs.m_PlaceboDebandRadius); m_pDS->exec(strSQL2); }
        ...
        if (list[i] == "PlaceboColorMapGamutMapping") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapGamutMapping         text NOT NULL DEFAULT  '%s'", vs.m_PlaceboColorMapGamutMapping == -1 ? "disabled" : pl_gamut_map_functions[vs.m_PlaceboColorMapGamutMapping]->description == nullptr ? "''" : pl_gamut_map_functions[vs.m_PlaceboColorMapGamutMapping]->description); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapToneMapping") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapToneMapping          text NOT NULL DEFAULT  '%s'", vs.m_PlaceboColorMapToneMapping == -1 ? "disabled" : pl_tone_map_functions[vs.m_PlaceboColorMapToneMapping]->description == nullptr ? "''" : pl_tone_map_functions[vs.m_PlaceboColorMapToneMapping]->description); m_pDS->exec(strSQL2); }
        ...
        if (list[i] == "PlaceboColorMapGamutExpansion") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapInverseToneMapping  bool NOT NULL DEFAULT    %i", vs.m_PlaceboColorMapInverseToneMapping); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapLut3dSizeI") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapLut3dSizeI          integer NOT NULL DEFAULT %i", vs.m_PlaceboColorMapLut3dSizeI); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapLut3dSizeC") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapLut3dSizeC          integer NOT NULL DEFAULT %i", vs.m_PlaceboColorMapLut3dSizeC); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapLut3dSizeH") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapLut3dSizeH          integer NOT NULL DEFAULT %i", vs.m_PlaceboColorMapLut3dSizeH); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapLut3dTricubic") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapLut3dTricubic       bool NOT NULL DEFAULT    %i", vs.m_PlaceboColorMapLut3dTricubic); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapLutSize") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapLutSize             integer NOT NULL DEFAULT %i", vs.m_PlaceboColorMapLutSize); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapShowClipping") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapShowClipping        bool NOT NULL DEFAULT    %i", vs.m_PlaceboColorMapShowClipping); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapIntent") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapIntent               text NOT NULL DEFAULT '%s'", CPLHelper::getColorMapIntentDescriptionFromIndex(vs.m_PlaceboColorMapIntent).c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapForceToneMappingLut") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapForceToneMappingLut bool NOT NULL DEFAULT    %i", vs.m_PlaceboColorMapForceToneMappingLut); m_pDS->exec(strSQL2); }

        if (list[i] == "PlaceboDeinterlaceEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDeinterlaceEnabled          bool NOT NULL DEFAULT    %i", vs.m_PlaceboDeinterlaceEnabled); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDeinterlaceAlgo") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDeinterlaceAlgo              text NOT NULL DEFAULT '%s'", CPLHelper::getDeinterlaceAlgoDescriptionFromIndex(vs.m_PlaceboDeinterlaceAlgo).c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDeinterlaceSkipSpatialCheck") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDeinterlaceSkipSpatialCheck bool NOT NULL DEFAULT    %i", vs.m_PlaceboDeinterlaceSkipSpatialCheck); m_pDS->exec(strSQL2); }

        if (list[i] == "PlaceboSigmoidEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSigmoidEnabled bool NOT NULL DEFAULT    %i", vs.m_PlaceboSigmoidEnabled); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboSigmoidCenter") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSigmoidCenter  float NOT NULL DEFAULT   %f", vs.m_PlaceboSigmoidCenter); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboSigmoidSlope") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboSigmoidSlope   float NOT NULL DEFAULT   %f", vs.m_PlaceboSigmoidSlope); m_pDS->exec(strSQL2); }

        if (list[i] == "PlaceboConeEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboConeEnabled    bool NOT NULL DEFAULT     %i", vs.m_PlaceboConeEnabled); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboConeCones") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboConeCones       text NOT NULL DEFAULT   '%s'", CPLHelper::getConeConesDescriptionFromIndex(vs.m_PlaceboConeCones).c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboConeStrength") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboConeStrength   float NOT NULL DEFAULT    %f", vs.m_PlaceboConeStrength); m_pDS->exec(strSQL2); }

        if (list[i] == "PlaceboDitherEnabled") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDitherEnabled  bool NOT NULL DEFAULT     %i", vs.m_PlaceboDitherEnabled); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDitherMethod") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDitherMethod    text NOT NULL DEFAULT   '%s'", CPLHelper::getDitherMethodDescriptionFromIndex(vs.m_PlaceboDitherMethod).c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDitherLutSize") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDitherLutSize  integer NOT NULL DEFAULT  %i", vs.m_PlaceboDitherLutSize); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDitherTemporal") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDitherTemporal bool NOT NULL DEFAULT     %i", vs.m_PlaceboDitherTemporal); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDitherTransfer") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDitherTransfer  text NOT NULL DEFAULT   '%s'", CPLHelper::getDitherTransferDescriptionFromIndex(vs.m_PlaceboDitherTransfer).c_str()); m_pDS->exec(strSQL2); }

        if (list[i] == "PlaceboToneConstantExposure") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantExposure         float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantExposure); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantKneeAdaptation") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantKneeAdaptation   float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantKneeAdaptation); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantKneeDefault") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantKneeDefault      float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantKneeDefault); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantKneeMaximum") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantKneeMaximum      float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantKneeMaximum); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantKneeMinimum") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantKneeMinimum      float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantKneeMinimum); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantKneeOffset") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantKneeOffset       float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantKneeOffset); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantLinearKnee") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantLinearKnee       float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantLinearKnee); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantReinhardContrast") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantReinhardContrast float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantReinhardContrast); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantSlopeOffset") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantSlopeOffset      float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantSlopeOffset); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantSlopeTuning") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantSlopeTuning      float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantSlopeTuning); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboToneConstantSplineContrast") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboToneConstantSplineContrast   float NOT NULL DEFAULT %f", vs.m_PlaceboToneConstantSplineContrast); m_pDS->exec(strSQL2); }
        ...
        if (list[i] == "PlaceboColorMapVisualizeRectX0") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapVisualizeRectX0 float NOT NULL DEFAULT %f", vs.m_PlaceboColorMapVisualizeRectX0); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapVisualizeRectX1") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapVisualizeRectX1 float NOT NULL DEFAULT %f", vs.m_PlaceboColorMapVisualizeRectX1); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapVisualizeRectY0") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapVisualizeRectY0 float NOT NULL DEFAULT %f", vs.m_PlaceboColorMapVisualizeRectY0); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapVisualizeRectY1") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapVisualizeRectY1 float NOT NULL DEFAULT %f", vs.m_PlaceboColorMapVisualizeRectY1); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapVisualizeHue") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapVisualizeHue    float NOT NULL DEFAULT %f", vs.m_PlaceboColorMapVisualizeHue); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboColorMapVisualizeTheta") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboColorMapVisualizeTheta  float NOT NULL DEFAULT %f", vs.m_PlaceboColorMapVisualizeTheta); m_pDS->exec(strSQL2); }

        if (list[i] == "PlaceboLutType") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboLutType                      text NOT NULL DEFAULT '%s'", CPLHelper::getLutTypeDescriptionFromIndex(vs.m_PlaceboLutType).c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboAntiringingStrength") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboAntiringingStrength          float NOT NULL DEFAULT   %f", vs.m_PlaceboAntiringingStrength); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboCorrectSubpixelOffset") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboCorrectSubpixelOffset        bool NOT NULL DEFAULT    %i", vs.m_PlaceboCorrectSubpixelOffset); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDisableBuiltinScalers") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDisableBuiltinScalers        bool NOT NULL DEFAULT    %i", vs.m_PlaceboDisableBuiltinScalers); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDisableDitherGammaCorrection") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDisableDitherGammaCorrection bool NOT NULL DEFAULT    %i", vs.m_PlaceboDisableDitherGammaCorrection); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDisableLinearScaling") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDisableLinearScaling         bool NOT NULL DEFAULT    %i", vs.m_PlaceboDisableLinearScaling); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboDynamicConstant") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboDynamicConstant              bool NOT NULL DEFAULT    %i", vs.m_PlaceboDynamicConstant); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboErrorDiffusion") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboErrorDiffusion               text NOT NULL DEFAULT '%s'", CPLHelper::getDiffusionKernelDescriptionFromIndex(vs.m_PlaceboErrorDiffusion).c_str()); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboForceDither") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboForceDither                  bool NOT NULL DEFAULT    %i", vs.m_PlaceboForceDither); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboForceLowBitDepthFbos") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboForceLowBitDepthFbos         bool NOT NULL DEFAULT    %i", vs.m_PlaceboForceLowBitDepthFbos); m_pDS->exec(strSQL2); }
        if (list[i] == "PlaceboIgnoreIccProfiles") { strSQL2 = PrepareSQL("ALTER TABLE settings ADD COLUMN PlaceboIgnoreIccProfiles            bool NOT NULL DEFAULT    %i", vs.m_PlaceboIgnoreIccProfiles); m_pDS->exec(strSQL2); }
  ...
  try
  {
    ...
    { // get the video settings info
	  ...
	  if (libplaceboSaveToDatabase)
      {
		...
		settings.m_PlaceboSkinZoomPosition = static_cast<VS_PLACEBO_SZ_POSITION> (m_pDS->fv("PlaceboSkinZoomPosition").get_asInt());
        ...
        settings.m_PlaceboDisplayHdrPeakLuminance = m_pDS->fv("PlaceboDisplayHdrPeakLuminance").get_asFloat();
		settings.m_PlaceboDisplaySdrPeakLuminance = m_pDS->fv("PlaceboDisplaySdrPeakLuminance").get_asFloat();
		settings.m_PlaceboTargetContrast = m_pDS->fv("PlaceboTargetContrast").get_asInt();
		settings.m_PlaceboNvRtxPipelineEnabled = m_pDS->fv("PlaceboNvRtxPipelineEnabled").get_asBool();
		settings.m_PlaceboNvSuperResolutionEnabled = m_pDS->fv("PlaceboNvSuperResolutionEnabled").get_asBool();
		settings.m_PlaceboNvRtxHdrEnabled = m_pDS->fv("PlaceboNvRtxHdrEnabled").get_asBool();
		settings.m_PlaceboSdrTargetContrast = m_pDS->fv("PlaceboSdrTargetContrast").get_asInt();
		...
		settings.m_PlaceboBrightnessSdrSdr = m_pDS->fv("PlaceboBrightnessSdrSdr").get_asFloat();
		settings.m_PlaceboContrastSdrSdr = m_pDS->fv("PlaceboContrastSdrSdr").get_asFloat();
		settings.m_PlaceboBrightnessHdrHdr = m_pDS->fv("PlaceboBrightnessHdrHdr").get_asFloat();
		settings.m_PlaceboContrastHdrHdr = m_pDS->fv("PlaceboContrastHdrHdr").get_asFloat();
		settings.m_PlaceboBrightnessHdrSdr = m_pDS->fv("PlaceboBrightnessHdrSdr").get_asFloat();
		...
		settings.m_PlaceboSdrColorMapGamutExpansion = m_pDS->fv("PlaceboSdrColorMapGamutExpansion").get_asBool();
		settings.m_PlaceboSdrColorMapIntent = (str = (m_pDS->fv("PlaceboSdrColorMapIntent").get_asString())) == "" ? -1 : CPLHelper::getColorMapIntentIndexFromDescription(str);
		settings.m_PlaceboSdrColorMapGamutMapping = (str = (m_pDS->fv("PlaceboSdrColorMapGamutMapping").get_asString())) == "" ? -1 : CPLHelper::getGamutMapIndexFromDescription(str);
		settings.m_PlaceboSdrColorMapToneMapping = (str = (m_pDS->fv("PlaceboSdrColorMapToneMapping").get_asString())) == "" ? -1 : CPLHelper::getToneMapIndexFromDescription(str);
		settings.m_PlaceboSdrToneConstantExposure = m_pDS->fv("PlaceboSdrToneConstantExposure").get_asFloat();
		settings.m_PlaceboSdrToneConstantKneeAdaptation = m_pDS->fv("PlaceboSdrToneConstantKneeAdaptation").get_asFloat();
		settings.m_PlaceboSdrToneConstantKneeDefault = m_pDS->fv("PlaceboSdrToneConstantKneeDefault").get_asFloat();
		settings.m_PlaceboSdrToneConstantKneeMaximum = m_pDS->fv("PlaceboSdrToneConstantKneeMaximum").get_asFloat();
		settings.m_PlaceboSdrToneConstantKneeMinimum = m_pDS->fv("PlaceboSdrToneConstantKneeMinimum").get_asFloat();
		settings.m_PlaceboSdrToneConstantKneeOffset = m_pDS->fv("PlaceboSdrToneConstantKneeOffset").get_asFloat();
		settings.m_PlaceboSdrToneConstantLinearKnee = m_pDS->fv("PlaceboSdrToneConstantLinearKnee").get_asFloat();
		settings.m_PlaceboSdrToneConstantReinhardContrast = m_pDS->fv("PlaceboSdrToneConstantReinhardContrast").get_asFloat();
		...
		settings.m_PlaceboSdrGamutConstantsSoftclipDesat = m_pDS->fv("PlaceboSdrGamutConstantsSoftclipDesat").get_asFloat();
		settings.m_PlaceboSdrGamutConstantsSoftclipKnee = m_pDS->fv("PlaceboSdrGamutConstantsSoftclipKnee").get_asFloat();
        ...
        settings.m_PlaceboHue = m_pDS->fv("PlaceboHue").get_asFloat();
        settings.m_PlaceboTemperature = m_pDS->fv("PlaceboTemperature").get_asFloat();

        settings.m_PlaceboPeakDetectEnabled = m_pDS->fv("PlaceboPeakDetectEnabled").get_asInt();
        settings.m_PlaceboPeakDetectSmoothingPeriod = m_pDS->fv("PlaceboPeakDetectSmoothingPeriod").get_asFloat();
        settings.m_PlaceboPeakDetectSceneThresholdLow = m_pDS->fv("PlaceboPeakDetectSceneThresholdLow").get_asFloat();
        settings.m_PlaceboPeakDetectSceneThresholdHigh = m_pDS->fv("PlaceboPeakDetectSceneThresholdHigh").get_asFloat();
        settings.m_PlaceboPeakDetectPercentile = m_pDS->fv("PlaceboPeakDetectPercentile").get_asFloat();
        settings.m_PlaceboPeakDetectBlackCutoff = m_pDS->fv("PlaceboPeakDetectBlackCutoff").get_asFloat();
        settings.m_PlaceboPeakDetectAllowDelayed = m_pDS->fv("PlaceboPeakDetectAllowDelayed").get_asInt();

        settings.m_PlaceboUpscaler = (str=(m_pDS->fv("PlaceboUpscaler").get_asString())) == "" ? -1 : CPLHelper::getFilterIndexFromDescription(str);
        settings.m_PlaceboDownscaler = (str = (m_pDS->fv("PlaceboDownscaler").get_asString())) == "" ? -1 : CPLHelper::getFilterIndexFromDescription(str);
        settings.m_PlaceboPlaneUpscaler = (str = (m_pDS->fv("PlaceboPlaneUpscaler").get_asString())) == "" ? -1 : CPLHelper::getFilterIndexFromDescription(str);
        settings.m_PlaceboPlaneDownscaler = (str = (m_pDS->fv("PlaceboPlaneDownscaler").get_asString())) == "" ? -1 : CPLHelper::getFilterIndexFromDescription(str);
        settings.m_PlaceboFrameMixer = (str = (m_pDS->fv("PlaceboFrameMixer").get_asString())) == "" ? -1 : CPLHelper::getFilterIndexFromDescription(str);

        settings.m_PlaceboDebandEnabled = m_pDS->fv("PlaceboDebandEnabled").get_asInt();
        settings.m_PlaceboDebandGrain = m_pDS->fv("PlaceboDebandGrain").get_asFloat();
        settings.m_PlaceboDebandGrainNeutral0 = m_pDS->fv("PlaceboDebandGrainNeutral0").get_asFloat();
        settings.m_PlaceboDebandGrainNeutral1 = m_pDS->fv("PlaceboDebandGrainNeutral1").get_asFloat();
        settings.m_PlaceboDebandGrainNeutral2 = m_pDS->fv("PlaceboDebandGrainNeutral2").get_asFloat();
        settings.m_PlaceboDebandIterations = m_pDS->fv("PlaceboDebandIterations").get_asInt();
        settings.m_PlaceboDebandRadius = m_pDS->fv("PlaceboDebandRadius").get_asFloat();
        settings.m_PlaceboDebandThreshold = m_pDS->fv("PlaceboDebandThreshold").get_asFloat();

        settings.m_PlaceboColorMapEnabled = m_pDS->fv("PlaceboColorMapEnabled").get_asInt();
        settings.m_PlaceboColorMapGamutMapping = (str = (m_pDS->fv("PlaceboColorMapGamutMapping").get_asString())) == "" ? -1 : CPLHelper::getGamutMapIndexFromDescription(str);
        settings.m_PlaceboColorMapToneMapping = (str = (m_pDS->fv("PlaceboColorMapToneMapping").get_asString())) == "" ? -1 : CPLHelper::getToneMapIndexFromDescription(str);
        settings.m_PlaceboColorMapContrastRecovery = m_pDS->fv("PlaceboColorMapContrastRecovery").get_asFloat();
        settings.m_PlaceboColorMapContrastSmoothness = m_pDS->fv("PlaceboColorMapContrastSmoothness").get_asFloat();
        settings.m_PlaceboColorMapGamutExpansion = m_pDS->fv("PlaceboColorMapGamutExpansion").get_asInt();
        settings.m_PlaceboColorMapInverseToneMapping = m_pDS->fv("PlaceboColorMapInverseToneMapping").get_asInt();
        settings.m_PlaceboColorMapLut3dSizeI = m_pDS->fv("PlaceboColorMapLut3dSizeI").get_asInt();
        settings.m_PlaceboColorMapLut3dSizeC = m_pDS->fv("PlaceboColorMapLut3dSizeC").get_asInt();
        settings.m_PlaceboColorMapLut3dSizeH = m_pDS->fv("PlaceboColorMapLut3dSizeH").get_asInt();
        settings.m_PlaceboColorMapLut3dTricubic = m_pDS->fv("PlaceboColorMapLut3dTricubic").get_asInt();
        settings.m_PlaceboColorMapLutSize = m_pDS->fv("PlaceboColorMapLutSize").get_asInt();
        settings.m_PlaceboColorMapShowClipping = m_pDS->fv("PlaceboColorMapShowClipping").get_asInt();
        settings.m_PlaceboColorMapIntent = (str = (m_pDS->fv("PlaceboColorMapIntent").get_asString())) == "" ? -1 : CPLHelper::getColorMapIntentIndexFromDescription(str);
        settings.m_PlaceboColorMapForceToneMappingLut = m_pDS->fv("PlaceboColorMapForceToneMappingLut").get_asInt();

        settings.m_PlaceboDeinterlaceEnabled = m_pDS->fv("PlaceboDeinterlaceEnabled").get_asInt();
        settings.m_PlaceboDeinterlaceAlgo = (str = (m_pDS->fv("PlaceboDeinterlaceAlgo").get_asString())) == "" ? -1 : CPLHelper::getDeinterlaceAlgoIndexFromDescription(str);
        ...
        settings.m_PlaceboConeEnabled = m_pDS->fv("PlaceboConeEnabled").get_asInt();
        settings.m_PlaceboConeCones = (str = (m_pDS->fv("PlaceboConeCones").get_asString())) == "" ? -1 : CPLHelper::getConeConesIndexFromDescription(str);
        ...
        settings.m_PlaceboDitherMethod = (str = (m_pDS->fv("PlaceboDitherMethod").get_asString())) == "" ? -1 : CPLHelper::getDitherMethodIndexFromDescription(str);
        ...
        settings.m_PlaceboDitherTransfer = (str = (m_pDS->fv("PlaceboDitherTransfer").get_asString())) == "" ? -1 : CPLHelper::getDitherTransferIndexFromDescription(str);

        settings.m_PlaceboToneConstantExposure = m_pDS->fv("PlaceboToneConstantExposure").get_asFloat();
        settings.m_PlaceboToneConstantKneeAdaptation = m_pDS->fv("PlaceboToneConstantKneeAdaptation").get_asFloat();
        settings.m_PlaceboToneConstantKneeDefault = m_pDS->fv("PlaceboToneConstantKneeDefault").get_asFloat();
        settings.m_PlaceboToneConstantKneeMaximum = m_pDS->fv("PlaceboToneConstantKneeMaximum").get_asFloat();
        settings.m_PlaceboToneConstantKneeMinimum = m_pDS->fv("PlaceboToneConstantKneeMinimum").get_asFloat();
        settings.m_PlaceboToneConstantKneeOffset = m_pDS->fv("PlaceboToneConstantKneeOffset").get_asFloat();
        settings.m_PlaceboToneConstantLinearKnee = m_pDS->fv("PlaceboToneConstantLinearKnee").get_asFloat();
        settings.m_PlaceboToneConstantReinhardContrast = m_pDS->fv("PlaceboToneConstantReinhardContrast").get_asFloat();
        settings.m_PlaceboToneConstantSlopeOffset = m_pDS->fv("PlaceboToneConstantSlopeOffset").get_asFloat();
        settings.m_PlaceboToneConstantSlopeTuning = m_pDS->fv("PlaceboToneConstantSlopeTuning").get_asFloat();
        settings.m_PlaceboToneConstantSplineContrast = m_pDS->fv("PlaceboToneConstantSplineContrast").get_asFloat();
        ...
        settings.m_PlaceboColorMapVisualizeRectX0 = m_pDS->fv("PlaceboColorMapVisualizeRectX0").get_asFloat();
        settings.m_PlaceboColorMapVisualizeRectX1 = m_pDS->fv("PlaceboColorMapVisualizeRectX1").get_asFloat();
        settings.m_PlaceboColorMapVisualizeRectY0 = m_pDS->fv("PlaceboColorMapVisualizeRectY0").get_asFloat();
        settings.m_PlaceboColorMapVisualizeRectY1 = m_pDS->fv("PlaceboColorMapVisualizeRectY1").get_asFloat();
        settings.m_PlaceboColorMapVisualizeHue = m_pDS->fv("PlaceboColorMapVisualizeHue").get_asFloat();
        settings.m_PlaceboColorMapVisualizeTheta = m_pDS->fv("PlaceboColorMapVisualizeTheta").get_asFloat();

        settings.m_PlaceboLutType = (str = (m_pDS->fv("PlaceboLutType").get_asString())) == "" ? -1 : CPLHelper::getLutTypeIndexFromDescription(str); settings.m_placeboOptions->getPlOptions()->params.lut = settings.m_PlaceboLutType == -1 ? nullptr: settings.m_PlaceboLut.get();
        settings.m_PlaceboAntiringingStrength = m_pDS->fv("PlaceboAntiringingStrength").get_asFloat();
        settings.m_PlaceboCorrectSubpixelOffset = m_pDS->fv("PlaceboCorrectSubpixelOffset").get_asInt();
        settings.m_PlaceboDisableBuiltinScalers = m_pDS->fv("PlaceboDisableBuiltinScalers").get_asInt();
        ...
        settings.m_PlaceboErrorDiffusion = (str = (m_pDS->fv("PlaceboErrorDiffusion").get_asString())) == "" ? -1 : CPLHelper::getErrorDiffusionIndexFromDescription(str);
        ...
        settings.m_PlaceboIgnoreIccProfiles = m_pDS->fv("PlaceboIgnoreIccProfiles").get_asInt();
...
void CVideoDatabase::SetVideoSettings(int idFile, const CVideoSettings &settings)
{
  try
  {
    if (nullptr == m_pDB)
    ...
    if (m_pDS->num_rows() > 0)
    {
      ...
      strSQL = PrepareSQL(
          ...
          static_cast<double>(settings.m_CustomVerticalShift), settings.m_AudioStream,
          settings.m_SubtitleStream, static_cast<double>(settings.m_SubtitleDelay),
          settings.m_SubtitleOn, static_cast<double>(settings.m_Brightness),
          static_cast<double>(settings.m_Contrast), static_cast<double>(settings.m_Gamma),
      ...
      strSQL2 = PrepareSQL("ResumeTime=%i,StereoMode=%i,StereoInvert=%i,VideoStream=%i,"
                           ...
                           settings.m_ResumeTime, settings.m_StereoMode, settings.m_StereoInvert,
                           settings.m_VideoStream, settings.m_ToneMapMethod,
                           static_cast<double>(settings.m_ToneMapParam), idFile);
      strSQL += strSQL2;
      m_pDS->exec(strSQL);
      m_pDS->close();
	  if (libplaceboSaveToDatabase)
      {
        strSQL = PrepareSQL(
		  ...
		  "PlaceboTargetContrast=%i,PlaceboNvRtxPipelineEnabled=%i,PlaceboNvSuperResolutionEnabled=%i,PlaceboNvRtxHdrEnabled=%i,PlaceboSdrTargetContrast=%i,PlaceboTargetColorspaceHint=%i,PlaceboTargetColorspaceHintMode=%i,"
          ...
          "PlaceboFrameMixer='%s',PlaceboDebandEnabled=%i,PlaceboDebandGrain=%f,PlaceboDebandGrainNeutral0=%f,"
          "PlaceboDebandGrainNeutral1=%f,PlaceboDebandGrainNeutral2=%f,PlaceboDebandIterations=%i,PlaceboDebandRadius=%f,"
          "PlaceboDebandThreshold=%f,PlaceboColorMapEnabled=%i,PlaceboColorMapGamutMapping='%s',PlaceboColorMapToneMapping='%s',"
          "PlaceboColorMapContrastRecovery=%f,PlaceboColorMapContrastSmoothness=%f,PlaceboColorMapGamutExpansion=%i,"
          "PlaceboColorMapInverseToneMapping=%i,PlaceboColorMapLut3dSizeI=%i,PlaceboColorMapLut3dSizeC=%i,PlaceboColorMapLut3dSizeH=%i,"
          "PlaceboColorMapLut3dTricubic=%i,PlaceboColorMapLutSize=%i,PlaceboColorMapShowClipping=%i,PlaceboColorMapIntent='%s',"
          "PlaceboColorMapForceToneMappingLut=%i,PlaceboDeinterlaceEnabled=%i,PlaceboDeinterlaceAlgo='%s',PlaceboDeinterlaceSkipSpatialCheck=%i,"
          "PlaceboSigmoidEnabled=%i,PlaceboSigmoidCenter=%f,PlaceboSigmoidSlope=%f,PlaceboConeEnabled=%i,"
          "PlaceboConeCones='%s',PlaceboConeStrength=%f,PlaceboDitherEnabled=%i,PlaceboDitherMethod='%s',PlaceboDitherLutSize=%i,"
          "PlaceboDitherTemporal=%i,PlaceboDitherTransfer='%s',PlaceboToneConstantExposure=%f,PlaceboToneConstantKneeAdaptation=%f,"
          ...
          static_cast<double>(settings.m_PlaceboDisplayHdrPeakLuminance),
		  static_cast<double>(settings.m_PlaceboDisplaySdrPeakLuminance),
		  settings.m_PlaceboTargetContrast,
		  settings.m_PlaceboNvRtxPipelineEnabled,
		  settings.m_PlaceboNvSuperResolutionEnabled,
		  settings.m_PlaceboNvRtxHdrEnabled,
		  settings.m_PlaceboSdrTargetContrast,
		  ...
		  settings.m_PlaceboBrightnessSdrSdr,
		  settings.m_PlaceboContrastSdrSdr,
		  settings.m_PlaceboBrightnessHdrHdr,
		  settings.m_PlaceboContrastHdrHdr,
		  settings.m_PlaceboBrightnessHdrSdr,
          ...
          static_cast<double>(settings.m_PlaceboSdrSaturation),
		  settings.m_PlaceboSdrColorMapInverseToneMapping,
		  settings.m_PlaceboSdrColorMapGamutExpansion,
		  CPLHelper::getColorMapIntentDescriptionFromIndex(settings.m_PlaceboSdrColorMapIntent).c_str(),
		  settings.m_PlaceboSdrColorMapGamutMapping == -1 ? "disabled" : pl_gamut_map_functions [settings.m_PlaceboSdrColorMapGamutMapping]->description == nullptr ? "''" : ((std::string) pl_gamut_map_functions [settings.m_PlaceboSdrColorMapGamutMapping]->description).c_str(),
		  settings.m_PlaceboSdrColorMapToneMapping == -1 ? "disabled" : pl_tone_map_functions [settings.m_PlaceboSdrColorMapToneMapping]->description == nullptr ? "''" : pl_tone_map_functions [settings.m_PlaceboSdrColorMapToneMapping]->description,
		  static_cast<double>(settings.m_PlaceboSdrToneConstantExposure),
		  static_cast<double>(settings.m_PlaceboSdrToneConstantKneeAdaptation),
		  static_cast<double>(settings.m_PlaceboSdrToneConstantKneeDefault),
		  static_cast<double>(settings.m_PlaceboSdrToneConstantKneeMaximum),
	      static_cast<double>(settings.m_PlaceboSdrToneConstantKneeMinimum),
		  static_cast<double>(settings.m_PlaceboSdrToneConstantKneeOffset),
		  static_cast<double>(settings.m_PlaceboSdrToneConstantLinearKnee),
		  static_cast<double>(settings.m_PlaceboSdrToneConstantReinhardContrast),
		  ...
		  static_cast<double>(settings.m_PlaceboSdrGamutConstantsSoftclipDesat),
		  static_cast<double>(settings.m_PlaceboSdrGamutConstantsSoftclipKnee),

          static_cast<double>(settings.m_PlaceboHue),
          static_cast<double>(settings.m_PlaceboTemperature),
          settings.m_PlaceboPeakDetectEnabled,
          static_cast<double>(settings.m_PlaceboPeakDetectSmoothingPeriod),
          static_cast<double>(settings.m_PlaceboPeakDetectSceneThresholdLow),
          static_cast<double>(settings.m_PlaceboPeakDetectSceneThresholdHigh),
          static_cast<double>(settings.m_PlaceboPeakDetectPercentile),
          static_cast<double>(settings.m_PlaceboPeakDetectBlackCutoff),
          settings.m_PlaceboPeakDetectAllowDelayed,
          settings.m_PlaceboUpscaler == -1 ? "disabled" : pl_filter_configs[settings.m_PlaceboUpscaler]->description == nullptr ? "''" : pl_filter_configs[settings.m_PlaceboUpscaler]->description,
          settings.m_PlaceboDownscaler == -1 ? "disabled" : pl_filter_configs[settings.m_PlaceboDownscaler]->description == nullptr ? "''" : pl_filter_configs[settings.m_PlaceboDownscaler]->description,
          settings.m_PlaceboPlaneUpscaler == -1 ? "disabled" : pl_filter_configs[settings.m_PlaceboPlaneUpscaler]->description == nullptr ? "''" : pl_filter_configs[settings.m_PlaceboPlaneUpscaler]->description,
          settings.m_PlaceboPlaneDownscaler == -1 ? "disabled" : pl_filter_configs[settings.m_PlaceboPlaneDownscaler]->description == nullptr ? "''" : pl_filter_configs[settings.m_PlaceboPlaneDownscaler]->description,
          settings.m_PlaceboFrameMixer == -1 ? "disabled" : pl_filter_configs[settings.m_PlaceboFrameMixer]->description == nullptr ? "''" : pl_filter_configs[settings.m_PlaceboFrameMixer]->description,
          settings.m_PlaceboDebandEnabled,
          static_cast<double>(settings.m_PlaceboDebandGrain),
          static_cast<double>(settings.m_PlaceboDebandGrainNeutral0),
          static_cast<double>(settings.m_PlaceboDebandGrainNeutral1),
          static_cast<double>(settings.m_PlaceboDebandGrainNeutral2),
          settings.m_PlaceboDebandIterations,
          ...
          settings.m_PlaceboColorMapInverseToneMapping,
          settings.m_PlaceboColorMapLut3dSizeI,
          settings.m_PlaceboColorMapLut3dSizeC,
          settings.m_PlaceboColorMapLut3dSizeH,
          settings.m_PlaceboColorMapLut3dTricubic,
          settings.m_PlaceboColorMapLutSize,
          settings.m_PlaceboColorMapShowClipping,
          CPLHelper::getColorMapIntentDescriptionFromIndex(settings.m_PlaceboColorMapIntent).c_str(),
          settings.m_PlaceboColorMapForceToneMappingLut,
          settings.m_PlaceboDeinterlaceEnabled,
          CPLHelper::getDeinterlaceAlgoDescriptionFromIndex(settings.m_PlaceboDeinterlaceAlgo).c_str(),
          settings.m_PlaceboDeinterlaceSkipSpatialCheck,
          settings.m_PlaceboSigmoidEnabled,
          static_cast<double>(settings.m_PlaceboSigmoidCenter),
          static_cast<double>(settings.m_PlaceboSigmoidSlope),
          settings.m_PlaceboConeEnabled,
          CPLHelper::getConeConesDescriptionFromIndex(settings.m_PlaceboConeCones).c_str(),
          static_cast<double>(settings.m_PlaceboConeStrength),
          settings.m_PlaceboDitherEnabled,
          CPLHelper::getDitherMethodDescriptionFromIndex(settings.m_PlaceboDitherMethod).c_str(),
          settings.m_PlaceboDitherLutSize,
          settings.m_PlaceboDitherTemporal,
          CPLHelper::getDitherTransferDescriptionFromIndex(settings.m_PlaceboDitherTransfer).c_str(),
          static_cast<double>(settings.m_PlaceboToneConstantExposure),
          static_cast<double>(settings.m_PlaceboToneConstantKneeAdaptation),
          static_cast<double>(settings.m_PlaceboToneConstantKneeDefault),
          static_cast<double>(settings.m_PlaceboToneConstantKneeMaximum),
          static_cast<double>(settings.m_PlaceboToneConstantKneeMinimum),
          static_cast<double>(settings.m_PlaceboToneConstantKneeOffset),
          static_cast<double>(settings.m_PlaceboToneConstantLinearKnee),
          static_cast<double>(settings.m_PlaceboToneConstantReinhardContrast),
          static_cast<double>(settings.m_PlaceboToneConstantSlopeOffset),
          static_cast<double>(settings.m_PlaceboToneConstantSlopeTuning),
          ...
          static_cast<double>(settings.m_PlaceboColorMapVisualizeRectX1),
          static_cast<double>(settings.m_PlaceboColorMapVisualizeRectY0),
          static_cast<double>(settings.m_PlaceboColorMapVisualizeRectY1),
          static_cast<double>(settings.m_PlaceboColorMapVisualizeHue),
          static_cast<double>(settings.m_PlaceboColorMapVisualizeTheta),
          CPLHelper::getLutTypeDescriptionFromIndex(settings.m_PlaceboLutType).c_str(),
          static_cast<double>(settings.m_PlaceboAntiringingStrength),
          settings.m_PlaceboCorrectSubpixelOffset,
          settings.m_PlaceboDisableBuiltinScalers,
          settings.m_PlaceboDisableDitherGammaCorrection,
          settings.m_PlaceboDisableLinearScaling,
          settings.m_PlaceboDynamicConstant,
          CPLHelper::getDiffusionKernelDescriptionFromIndex(settings.m_PlaceboErrorDiffusion).c_str(),
          settings.m_PlaceboForceDither,
          settings.m_PlaceboForceLowBitDepthFbos,
          settings.m_PlaceboIgnoreIccProfiles,
        ...
        std::string serializedShadersData;
        CPLHelper::SerializeShaders(settings, serializedShadersData);
        strSQL = PrepareSQL("update settings set PlaceboShadersData='%s' where idFile=%i\n", serializedShadersData.c_str(), idFile);
        m_pDS->exec(strSQL);
        m_pDS->close();
      }
    }
    ...
    { // add the items
      ...
      strSQL += PrepareSQL(
        ...
        idFile, settings.m_InterlaceMethod, settings.m_ViewMode,
          ...
          static_cast<double>(settings.m_Contrast), static_cast<double>(settings.m_Gamma),
          static_cast<double>(settings.m_VolumeAmplification),
          static_cast<double>(settings.m_AudioDelay), settings.m_ResumeTime,
          static_cast<double>(settings.m_Sharpness), static_cast<double>(settings.m_NoiseReduction),
